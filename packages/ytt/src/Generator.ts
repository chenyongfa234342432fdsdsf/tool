...
export class Generator {
  ...
  async write(outputFileList: OutputFileList) {
    return Promise.all(
      Object.keys(outputFileList).map(async outputFilePath => {
        ...
        // TODO-LEO check if we need add some more
        // 始终写入主文件
  ...
  async generateInterfaceCode(
    ...
    let responseDataType = await jsonSchemaToType(
      responseDataJsonSchema,
      responseDataTypeName,
    )

    // 自动生成的 api 请求
    const apiRequestString = `
    
    // 以下为自动生成的 api 请求，需要使用的话请手动复制到相应模块的 api 请求层
    // import request, { MarkcoinRequest } from '@/plugins/request'

    // /** 
    // * [${interfaceInfo.title}↗](${interfaceInfo._url})
    // **/
    // export const ${interfaceInfo.method.toLowerCase()}${requestDataTypeName.replace(/Yapi/i, '').replace(capitalizeFirstLetter(interfaceInfo.method), '')}: MarkcoinRequest<
    //   ${requestDataTypeName},
    //   ${responseDataTypeName}['data']
    // > = ${interfaceInfo.method === 'GET' ? 'params' : 'data'} => {
    //   return request({
    ...
    // }
    `
    responseDataType += apiRequestString
