...
export function jsonSchemaToJSTTJsonSchema(
  ...
  return traverseJsonSchema(jsonSchema, (jsonSchema, currentPath) => {
    ...
    if (!jsonSchema.title) {
      // use property name as part of the title if not exist
      jsonSchema.title = currentPath[0] ? currentPath[0].toString() : jsonSchema.description 
      jsonSchema.title = capitalizeFirstLetter(jsonSchema.title || 'Undefined')
    }

    if (
      // ignore the first level then flattern the object type
      Array.isArray(currentPath) && currentPath.length > 0 &&
      jsonSchema.type === 'object' &&
      jsonSchema.properties &&
      Object.keys(jsonSchema.properties).length > 0
    ) {
      let aggreagedName = typeName
        .replaceAll(/ApiResponse/gi, '')
        .replaceAll(/list/gi, '')
        .replaceAll(/data/gi, '')

      const arr = currentPath as []
      let l = currentPath.length

      while (l) {
        aggreagedName += capitalizeFirstLetterOnly(arr[l - 1])
        l--
      }

      jsonSchema.title = capitalizeFirstLetterOnly(
        aggreagedName
          .replaceAll(/datadata/gi, 'Data')
          .replaceAll(/listlist/gi, 'List'),
      )
...
export async function jsonSchemaToType(
  ...
  // easy for debugger
  // if (typeName === 'V1IeoListIeoWalletApiResponse') {
  // if (typeName === 'YapiGetV1AssetOverviewApiResponse') {
  if (typeName === 'YapiGetSpotMarketRestV1NoticeListApiResponse') {
    debugger
  }

  // jsonSchema to typescript logic
  // TODO-LEO require seems not working
  ...
  const replced = code
    .replace(fakeTypeName, typeName)
    .trim()
    .replaceAll(/export interface (?!Yapi)/gi, 'export interface Yapi')
...
export function capitalizeFirstLetterOnly(str: string | number) {
  if(str === 0 ){
    // must be a list
    return 'List'
  }
