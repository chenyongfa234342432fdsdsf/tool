...
export const getBaseEnvS3Url = (mode, type, extType = ExtTypesEnum.default) => {
  ...
  if (mode === EnvTypesEnum.test) {
    if(extType===ExtTypesEnum.im){
      return getFastUrl(`https://im-staging-s3-saas.s3.ap-southeast-1.amazonaws.com/${type}`)
    }
...
export const getEnvS3MaintenanceApiUrl = (mode: EnvTypesEnum, businessId) => {
  switch (mode) {
    case EnvTypesEnum.production:
      return getFastUrl(
        `https://newbit-prod-s3-saas.s3.ap-northeast-1.amazonaws.com/${businessId}_maintenanceConfig.json`
      )
    case EnvTypesEnum.dev:
      return getFastUrl(
        `https://newbit-dev-s3-saas.s3.ap-southeast-1.amazonaws.com/${businessId}_maintenanceConfig.json`
      )
    case EnvTypesEnum.test:
      return getFastUrl(
        `https://newbit-staging-s3-saas.s3.ap-southeast-1.amazonaws.com/${businessId}_maintenanceConfig.json`
      )
    default:
      return getFastUrl(
        `https://newbit-dev-s3-saas.s3.ap-southeast-1.amazonaws.com/${businessId}_maintenanceConfig.json`
